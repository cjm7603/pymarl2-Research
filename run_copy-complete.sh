...
times=${7:-1}  # could change to 1 and only run 1 seed for each unit type as well
        ...
        for map in "${maps[@]}"; do
            for unit in "${units[@]}"; do
                for((i=0;i<times;i++)); do
                    ...
                    gpu=${gpus[$(($count % ${#gpus[@]}))]}
                    python3 src/main.py --config="$config" --env-config="$map" with group="$group" env_args.capability_config.n_units=$unit env_args.capability_config.n_enemies=$unit use_wandb=True td_lambda=$tdlambda epsilon_anneal_time=$epsanneal save_model=True "${args[@]}" &
